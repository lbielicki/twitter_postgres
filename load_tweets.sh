...
for file in $files; do
    ...
    python3 load_tweets.py --db=postgresql://postgres:pass@localhost:1855 --inputs "$file"
...
for file in $files; do
    ...
    unzip -p "$file" | sed 's/\\u0000//g' | psql "postgresql://postgres:pass@localhost:1854" -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"
done
